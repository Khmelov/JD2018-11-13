...
class Connect {
    ...
    static void reset() {
        ...
        try (Connection connection = DriverManager.getConnection(CONFIG.URL, CONFIG.USER, CONFIG.PASSWORD))  // CONFIG.URL - нужная база еще не создана
        {
            ...
            statement.executeUpdate("INSERT INTO `zakharnk`.`users` (`id`, `username`, `password`, `email`, `fullname`, `phone`, `address`, `roles_id`) " +
            ...
            //added item
            statement.executeUpdate("INSERT INTO `zakharnk`.`items` (`id`, `title`, `floors`, `square`, `materials`, `price`) " +
