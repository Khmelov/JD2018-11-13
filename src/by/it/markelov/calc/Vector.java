...
class Vector extends Var {
    ...
    public Var sub(Var other) throws CalcExeption {
        if (other instanceof Vector) {
            ...
            if (this.value.length != operand2.value.length)
                throw new CalcExeption("Вектора имеют разные (несовместимые) размеры");
            double mas[] = new double[this.value.length];
            for (int i = 0; i < this.value.length; i++) {
    ...
    public Var div(Var other) throws CalcExeption {
        if (other instanceof Vector)
            throw new CalcExeption("Деление вектора на вектор невозможно");
        if (other instanceof Matrix)
            throw new CalcExeption("Деление вектора на матрицу невозможно");
        if (other instanceof Scalar) {
            Scalar operand2 = (Scalar) other;
            double[] mas = new double[this.value.length];
            for (int i = 0; i < this.value.length; i++) {
