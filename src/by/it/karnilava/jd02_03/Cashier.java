...
//                   System.out.println(currentBuyer + " goes to " + this.getName());
                ...
                // Блок кода для: Если без текущего кассира на остальных будет меньше либо равно 5 человек на одного, то касса закрывается
                synchronized (lock) {
    ...
    // ЗДЕСЬ САМ МЕТОД ДЛЯ ПЕЧАТИ ТАБЛИЦЫ ДЛЯ ЧАСТИ С
    ...
    public void printInfo(int numero, Buyer currentBuyer) {
        ...
        switch (numero) {
            ...
            case 1: {
                ...
                int s = QueueOfPensioneers.queueOfPensioneers.size() + QueueToCashier.queueToCashier.size();
                System.out.printf("%40s%40s%40s%40s%40s%40s%40s%n", "Check is " + sum + " dollars" + delim, delim, delim, delim, delim, s + delim, grossIncome + delim);
                break;
            }
            ...
            case 2: {
                ...
                grossIncome.getAndAdd(sum);
                int s = QueueOfPensioneers.queueOfPensioneers.size() + QueueToCashier.queueToCashier.size();
                System.out.printf("%40s%40s%40s%40s%40s%40s%40s%n", delim, "Check is " + sum + " dollars" + delim, delim, delim, delim, s + delim, grossIncome + delim);
                break;
            }
            ...
            case 3: {
                ...
                grossIncome.getAndAdd(sum);
                int s = QueueOfPensioneers.queueOfPensioneers.size() + QueueToCashier.queueToCashier.size();
                System.out.printf("%40s%40s%40s%40s%40s%40s%40s%n", delim, delim, "Check is " + sum + " dollars" + delim, delim, delim, s + delim, grossIncome + delim);
                break;
            }
            ...
            case 4: {
                ...
                grossIncome.getAndAdd(sum);
                int s = QueueOfPensioneers.queueOfPensioneers.size() + QueueToCashier.queueToCashier.size();
                System.out.printf("%40s%40s%40s%40s%40s%40s%40s%n", delim, delim, delim, "Check is " + sum + " dollars" + delim, delim, s + delim, grossIncome + delim);
                break;
            }
            ...
            case 5: {
                ...
                grossIncome.getAndAdd(sum);
                int s = QueueOfPensioneers.queueOfPensioneers.size() + QueueToCashier.queueToCashier.size();
                System.out.printf("%40s%40s%40s%40s%40s%40s%40s%n", delim, delim, delim, delim, "Check is " + sum + " dollars" + delim, s + delim, grossIncome + delim);
                break;
            }
